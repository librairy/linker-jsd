      ...
      for ((run, i) <- activeRuns.zipWithIndex) {
        ...
        while (j < k) {
          ...
          if (count != 0) {
            ...
            if (JSKMeans.distance(newCenter, centers(run)(j)) > epsilon * epsilon) {
              changed = true
            }
...
//      if (lowerBoundOfSqDist < bestDistance) {
        val dist: Double = distance(center, point)
...
//      }
  ...
  /**
    ...
    */
  private[clustering] def distance(
                                               v1: VectorWithNorm,
                                               v2: VectorWithNorm): Double = {
    //MLUtils.fastSquaredDistance(v1.vector, v1.norm, v2.vector, v2.norm)
    JensenShannonDivergence.apply(v1.vector.toArray, v2.vector.toArray)
    //    Math.abs(JensenShannonSimilarity.apply(v1.vector.toArray, v2.vector.toArray)-1.0)
